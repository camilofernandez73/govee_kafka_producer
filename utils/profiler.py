...
class MyProfiler:
    ...
    def report_rate(self, time_metric_name, total_processed, unit):
        ...
        if time_delta > 0:
            ...
            logging.info(f"{time_metric_name}: processed {total_processed} {unit} in {time_delta:.4f} seconds ({rate:.2f} {unit}/s).")
        else:
            rate = total_processed
            logging.info(f"{time_metric_name}: processed {total_processed} {unit} in less than {time_delta:.4f} seconds.")
        


    def delete_temporal_metrics(self):
        keys = [key for key, value in self.time_metrics.items() if value.get('kind') == 'temporal']
        for kname in keys:
    ...
    def set_heartbeat_signal(self,  heartbeat_counter, heartbeat_time_metric, heartbeat_threshold, heartbeat_total):
    ...
    def eval_heartbeat(self, current_count):
        ...
        if modulo == 0:
            heartbeat_elapsed_time = time.monotonic() - self.time_metrics[self.heartbeat_time_metric]['start_time']
            heartbeat_rate = self.heartbeat_threshold / heartbeat_elapsed_time
            logging.info(f"Current {current_count}/{self.heartbeat_total}. Last {self.heartbeat_threshold} processed in {heartbeat_elapsed_time:.2f} seconds ({heartbeat_rate:.2f}/s). ")

        elif self.heartbeat_total > self.heartbeat_threshold and current_count == self.heartbeat_total:
            heartbeat_elapsed_time = time.monotonic() - self.time_metrics[self.heartbeat_time_metric]['start_time']
            heartbeat_rate = modulo / heartbeat_elapsed_time
            logging.info(f"Current {current_count}/{self.heartbeat_total}. Last {modulo} processed in {heartbeat_elapsed_time:.2f} seconds ({heartbeat_rate:.2f}/s). ")
