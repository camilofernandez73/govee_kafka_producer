    ...
    try:
        while True:
            ...
            if total_entries > 0:
                ...
                mp.set_heartbeat_signal('Entries','Last Batch', heartbeat_signal_lines, total_entries)
            else:
                logging.debug(f"Nothing to send. Next try in {iteration_delay} seconds.")
            ...
            for entry in entries:
                ...
                try:
                    ...
                    if not is_current: mp.counter_increment('Discarded')
                    ...
                    if set_offset:
                        offset_manager.update_offset(entry['filename'], entry['offset'] + len(entry['line']) + 1, entry_ts)
